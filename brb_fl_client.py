...
# Load the CSV file
file_path = 'C:\\Users\\ninzi\\Documents\\Master Thesis\\Code\\Alzheimers_CNN\\ADNI_AXIAL\\brb\\BRB_PSO\\selected_columns.csv'
...
output_dir = 'C:\\Users\\ninzi\\Documents\\Master Thesis\\Code\\Alzheimers_CNN\\ADNI_AXIAL\\brb\\BRB_PSO\\output'
...
for i in range(3):
    ...
    # Save client results
    client_train_mse, client_train_predicted = evaluate_model(optimized_parameters_pso, client_data[i])
    client_test_mse, client_test_predicted = evaluate_model(optimized_parameters_pso, test)

    client_train_results = pd.DataFrame({'Observed': client_data[i]['Group'], 'Predicted': client_train_predicted})
    client_test_results = pd.DataFrame({'Observed': test['Group'], 'Predicted': client_test_predicted})

    client_train_results.to_csv(os.path.join(output_dir, f'client_{i+1}_train_results.csv'), index=False)
    client_test_results.to_csv(os.path.join(output_dir, f'client_{i+1}_test_results.csv'), index=False)
...
# Save the final optimized parameters to CSV
...
final_params_df.to_csv(os.path.join(output_dir, 'final_optimized_parameters.csv'), index=False)
