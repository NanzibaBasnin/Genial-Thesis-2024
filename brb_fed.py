...
# Load the CSV file
file_path = 'C:\\Users\\HP\\Documents\\Discertation\\Code\\brb_fed_GE\\brb_fed_GE\\selected_columns.csv'
...
output_dir = 'C:\\Users\\HP\\Documents\\Discertation\\Code\\brb_fed_GE\\output'
...
def fitness_function(parameters, train_data):
    return objective_function(parameters, train_data)

def selection(population, fitnesses, num_parents):
    parents = np.empty((num_parents, population.shape[1]))
    for parent_idx in range(num_parents):
        min_fitness_idx = np.argmin(fitnesses)
        parents[parent_idx, :] = population[min_fitness_idx, :]
        fitnesses[min_fitness_idx] = np.inf  # To ensure this parent is not selected again
    return parents
    ...
    for idx in range(offspring_crossover.shape[0]):
        for gene_idx in range(offspring_crossover.shape[1]):
            if np.random.rand() < mutation_rate:
                offspring_crossover[idx, gene_idx] = np.random.uniform(0, 1)
    return offspring_crossover

# Integrate Genetic Algorithm as the Aggregator
def genetic_algorithm(client_parameters, client_data, num_generations=50, population_size=10, num_parents_mating=5):
    # Initialize population with random individuals
    population = np.array(client_parameters)
    while population.shape[0] < population_size:
        random_individual = np.random.uniform(low=0, high=1, size=(1, 38))
        population = np.vstack((population, random_individual))

    for generation in range(num_generations):
        fitnesses = np.array([fitness_function(individual, client_data) for individual in population])
    ...
    fitnesses = np.array([fitness_function(individual, client_data) for individual in population])
    best_match_idx = np.argmin(fitnesses)
    best_parameters = population[best_match_idx, :]
...
for iteration in range(num_iterations):
    ...
    # Use Genetic Algorithm for aggregation
    best_parameters = genetic_algorithm(optimized_parameters_clients, train, num_generations, population_size, num_parents_mating)
    best_parameters = ensure_belief_degrees(best_parameters)

    # Run PSO again on the best parameters to get the final optimized parameters
    print("Starting final PSO on best parameters for next iteration...")
    start_time = time.time()
    ...
    print("\nFinal Optimized Parameters after Genetic Algorithm:")
    print("Belief Degrees:")
